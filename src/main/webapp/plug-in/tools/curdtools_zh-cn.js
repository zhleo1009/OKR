 ...
 * @param id//主键字段
 */
// update-begin--Author:luobaoli Date:20150705
// for：增加了一个判断是否需要转换为restful风格URL的参数，如果该参数为true，那么转换为restful风格
...
// update-end--Author:luobaoli Date:20150705
// for：增加了一个判断是否需要转换为restful风格URL的参数，如果该参数为true，那么转换为restful风格
...
// }
	if (!rowsData || rowsData.length == 0) {
		top.layer.msg('请选择查看项目');
		return;
	}
	if (rowsData.length > 1) {
		top.layer.msg('请选择一条记录再查看');
		return;
	}
 ...
 * @return
 */
function deleteALLSelect(title,url,gname,idFileName) {
	gridname=gname;
    var ids = [];
 ...
 * @returns
 *//*
function createLayer(title, addurl,width,height) {
	...
	top.layer.open({
	      ...
	      yes: function(index, layero){
	    	    //do something
	    	  console.debug($(window.frames['iframe_'].contentWindow));
	    	  iframe = $(window).iframe[0].contentWindow;
	    	  saveObj();
	      }
	    });
...
function createdialog(title, content, url,name) {
	...
	top.layer.confirm(content, function(index){
		doSubmit(url,name);
		rowid = '';
		top.layer.close(index);
	});
...
function addOneTab(subtitle, url, icon) {
	...
	if(indexStyle=='sliding'||indexStyle=='bootstrap'){
		// shortcut和bootstrap和lte风格的tab跳转改为直接跳转
		window.location.href=url;
	}else if(indexStyle=='acele'||indexStyle=='ace'||indexStyle=='lte'){// bk
																		// acele、ace、lte风格开打tab页
		var id = "";
		// if(url.indexOf("=")!=-1){
		...
		// by zbk
		/*
		 * window.top.$.messager.progress({ text : '页面加载中....', interval : 300
		 * });
		 */
