...
<script src="plug-in/easyui_1.4.4/extends/validatebox.js"></script>
<script type="text/javascript">
	  ...
	  function initMainTableSourceSelect(){
		...
		//update-begin--Author: jg_renjie  Date:20151210 for：【自定义表单】表单主数据源不允许录入，只允许选择
		  var options = "<option value=''>--请选择表单数据源--</option>";
	 ...
	 $(function(){
		...
		/*  //自定义easyui表单验证规则
		 ...
		 $("#createCode").click(function(){
			 ...
			 if(formdbId == ''){
				 top.layer.msg('请选择表单数据源');
			 }
			 
			 var styleId = getStyleId();
			 if(styleId == ''){
				 top.layer.msg('请选择自定义表单模板');
			 }
	 ...
	 function doAdd(){
		 ...
		 top.layer.confirm('自定义表单保存后才能添加数据源，是否保存？', function(index){
			 ...
			 $('#formobj').form('submit', {
		    		...
		    		onSubmit : function() {
		    	       ...
		    	       var valid = $("#formobj").form('validate');
		    	       if(valid){
		    	    	   $.ajax({
		    	 			  type: 'POST',
		    	 			  async: false,
		    	 			  url: 'autoFormController.do?checkTbCode',
		    	       ...
		    	       return valid;
			    	   // return $("#formobj").form('validate');//验证表单
		    		},
		    		success : function(data) {
		    			var d = $.parseJSON(data);
						if (d.success) {
...
<div data-options="region:'west',title:'表单数据源',split:true,maxWidth:187" style="width: 200px;">
	<div style="width: 100%; height: 300px; overflow-y: auto;">
		<a href="#" class="easyui-linkbutton l-btn"  plain="true" iconcls="icon-add" onclick="javascript:doAdd();"></a>	
		<a href="#" class="easyui-linkbutton l-btn"  plain="true" iconcls="icon-edit" onclick="javascript:doUpdate();"></a>	
		<a href="#" class="easyui-linkbutton l-btn"  plain="true" iconcls="icon-remove" onclick="javascript:doDelete();"></a>	
		<a href="#" class="easyui-linkbutton l-btn"  plain="true" iconcls="icon-reload" onclick="javascript:doReload();"></a>	
		<ul id="dbSelect" class="ztree"></ul><!-- 数据源tree -->
	</div>
</div>
<div data-options="region:'center',title:'表单内容'" style="padding: 5px; background: #eee;">
<%-- <t:formvalid formid="formobj" dialog="false" beforeSubmit="setContent()" layout="table" action="autoFormController.do?doUpdate" tiptype="lte" > --%>

<form id="formobj" onsubmit="setContent()" action="autoFormController.do?doUpdate" method="post">
	...
	<!-- <table style="width: 100%;" cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value" style="width: 20%;">
				<input id="formName" name="formName" type="text" class="inputxt" style="width: 75%; height: 17px;" errorMsg="不能为中文" nullMsg="请填写表单编码！"  ajaxurl="autoFormController.do?checkTbCode" datatype="/^[A-Za-z\d-._]+$/" > 
			</td>
			<td align="right" style="width: 8%;">
				<label class="Validform_label"> 表单名: </label>
			</td>
			<td class="value" style="width: 20%;">
				<input id="formDesc"  name="formDesc" type="text" style="width: 75%; height: 17px;" class="inputxt" datatype="*" nullMsg="请填写表单名！">
			</td>
	...
	<table style="width: 100%;" cellpadding="0" cellspacing="1" class="formtable">
		<tr >
			...
			<td class="value" style="width: 20%;">
				<input id="formName" name="formName" type="text" style="width: 75%;margin-bottom: 4px;" class="easyui-validatebox" required="true" invalidMessage="不能为中文"  ajaxurl="autoFormController.do?checkTbCode" validType="NotCHS"  missingMessage="表单编码必须填写！"> 
				<span class="Validform_checktip"></span> <label class="Validform_label" style="display: none;">表单编码</label></td>
			<td align="right" style="width: 8%;">
				<label class="Validform_label"> 表单名: </label></td>
			<td class="value" style="width: 20%;">
				<input id="formDesc" required="true" name="formDesc" type="text" style="width: 75%;margin-bottom: 4px;" class="easyui-validatebox" missingMessage="表单名必须填写！"> <span class="Validform_checktip"></span> <label class="Validform_label" style="display: none;">表单名</label>
			</td>
			<td align="right" style="width: 8%;"><label class="Validform_label"> 表单数据源: </label></td>
			<td class="value" style="width: 20%;">
				<select id="mainTableSource" name="mainTableSource"></select> <span class="Validform_checktip"></span> <label class="Validform_label" style="display: none;">表单名</label>
			</td>
		</tr>
		<tr>
			<td class="value" colspan=6><input id="formContent" name="formContent" type="hidden"> 
	   ...
	   parse_form:function(template,fields)
	    {
		 //正则  radios|checkboxs|select 匹配的边界 |--|  因为当使用 {} 时js报错 (plugins|fieldname|fieldflow)
	        ...
	        template.replace(preg, function(plugin,p1,p2,p3,p4,p5,p6){
	            ...
	            /*top.layer.msg(JSON.stringify(parse_attr));return;*/
	    ...
	    fnCheckForm : function ( type ) {
	        ...
	        if(formEditor.hasContents()){
	            ...
	            $.ajax({
	                ...
	                success : function(data){
						if(data == true) {
							top.layer.msg('表单保存成功');
							window.location.href='${ctx}/config/form';
						} else {
							top.layer.msg('表单保存失败');
						}
	                }
	            });

	        } else {
	            top.layer.msg('表单内容不能为空！')
