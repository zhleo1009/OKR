...
<t:datagrid sortName="createDate" sortOrder="desc" name="tablePropertyList" title="smart.form.config" 
            ...
            queryMode="group" checkbox="false" >
	...
	<!-- table list -->
	<t:dgCol title="table.name" field="tableName" query="true" autocomplete="true"  width="200"/>
	<t:dgCol title="form.category" field="jformCategory" dictionary="bdfl"></t:dgCol>
	<t:dgCol title="table.description" field="content" width="200"></t:dgCol>
	...
	<t:dgCol title="sync.db" field="isDbSynch" replace="has.sync_Y,have.nosync_N" style="background:red;_N" query="true"></t:dgCol><%-- 同步数据库 --%>
	...
	<t:dgToolBar title="字段模板下载" icon="icon-put" url="export/template/online_import_template.xls" funname="doColTemplate"></t:dgToolBar>
...
<script type="text/javascript">
    ...
    //字段模板下载
	function doColTemplate(title,url){
	...
	function delCgForm(id,name){
		...
		top.layer.confirm('<t:mutiLang langKey="confirm.delete.record"/>？', function(index){
			checkIsExit(id,name);
	...
	function checkIsExit(id,name){
		$.post("cgFormHeadController.do?checkIsExit&&name="+name,function(data){
			...
			if (d.success) {
				...
				top.layer.confirm('<t:mutiLang langKey="table.exit.in.db.confirm"/>？', function(index){
					delThis(id);
					top.layer.close(index);
				});
	...
	function addForm(title,url,id){
		gridname=id;
		createwindow(title,url,900,600);
	}
	...
	function updateForm(title,url,id){
		...
		if (!rowsData || rowsData.length==0) {
			top.layer.msg('<t:mutiLang langKey="common.please.select.edit.item"/>');
			return;
		}
		if (rowsData.length>1) {
			top.layer.msg('<t:mutiLang langKey="common.please.select.one.record.to.edit"/>');
			return;
		}
	...
	function generate(title,url,id){
		...
		if("3" == rowsData[0].jformType){
			top.layer.msg('<t:mutiLang langKey="slave.table.can.not.generate.code"/>');
			return;
		}

		//未激活的表，不允许生成代码
		if("N" == rowsData[0].isDbSynch){
			top.layer.msg('<t:mutiLang langKey="please.syncdb"/>');
			return;
		}
	...
	function importFields(id,content) {
		openuploadwin('导入字段' + '[' + content +  ']', 'cgFormHeadController.do?upload&id='+id, "tablePropertyList");
	}
</script>
