...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" beforeSubmit="deleteUnUsedFiled();" layout="table" tiptype="lte" action="cgFormHeadController.do?save">
	<!-- tiptype="1" -->
	...
	<table cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value"><input class="inputxt" id="content" name="content" value="${cgFormHeadPage.content}" datatype="s2-100" nullmsg=<t:mutiLang langKey="please.input.table.desc"/>> <span class="Validform_checktip"></span></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td align="right">
				<label class="Validform_label"> <t:mutiLang langKey="form.template.style_pc" />:</label>
			</td>
			<td class="value">
				<%-- <select id="formTemplate" name="formTemplate" temVal="${cgFormHeadPage.formTemplate}">
				</select> --%>
				<select id="formTemplate" name="formTemplate" temVal="${cgFormHeadPage.formTemplate}">
					<option value="default">EASY默认模板</option>
				</select>
			</td>
			
			
			<td align="right">
				<label class="Validform_label"> <t:mutiLang langKey="form.template.style_mobile" />:</label>
			</td>
			<td class="value">
				<%-- <select id="formTemplateMobile" name="formTemplateMobile" temVal="${cgFormHeadPage.formTemplateMobile}">
				</select> --%>
				<select id="formTemplateMobile" name="formTemplateMobile" temVal="${cgFormHeadPage.formTemplateMobile}">
					<option value="default">EASY默认模板</option>
				</select>
  ...
  <!-- tabs -->
  <div id="tabs" class="easyui-tabs" tabPosition="top" fit="false" style="margin: 0px; padding: 0px; overflow: hidden; width: 100%;">
    <div title= '<t:mutiLang langKey="database.property"/>' width="auto" style="width: auto; margin: 0px; padding: 0px; overflow: hidden;">
      <div style="height: 25px;" class="datagrid-toolbar">
       <a id="addColumnBtn" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="addColumnBtnClick();" href="#"><t:mutiLang langKey="common.add.to"/></a>
       <a id="delColumnBtn" class="easyui-linkbutton" data-options="iconCls:'icon-remove'" onclick="delColumnBtnClick();" href="#"><t:mutiLang langKey="common.delete"/></a></div>
...
$(function() {
	...
	//getFormTemplateName(); // by zbk
	//getFormTemplateName_mobile(); // by zbk
}); 
...
//获取表单风格模板名称
function getFormTemplateName_mobile(){
 var type=$("#jformType").val();
	$.ajax({
		url:"${pageContext.request.contextPath}/cgformTemplateController.do?getTemplate",
		type:"post",
