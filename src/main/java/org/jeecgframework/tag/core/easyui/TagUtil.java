...
//		JSONObject jsonObject = JSONObject.fromObject(map);
	 ...
	 * @param dataGrid
	 */
	public static void printWriterJson(HttpServletResponse response,DataGrid dataGrid) {
		Map<String, Object> result=new HashMap<String, Object>();
		result.put("rows", dataGrid.getResults());
		result.put("total", dataGrid.getTotal());
		response.setContentType("application/json");
		response.setHeader("Cache-Control", "no-store");
		String results=JSONObject.toJSONString(result,filter);
		try {
			PrintWriter pw=response.getWriter();
			pw.write(results);
			pw.flush();
