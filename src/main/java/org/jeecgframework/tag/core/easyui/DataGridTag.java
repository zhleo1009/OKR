...
public class DataGridTag extends TagSupport {
	...
	private boolean collapsible=false;//是否允许收起
	//private boolean frozenColumn=false; // 是否是冰冻列    默认不是
	private String langArg;
	
	private String onCheck;//选择多选框条用的方法
	private String onUncheck;//取消多选框条用的方法
	private String onCheckAll;
	private String onUncheckAll;
	
	private boolean onloadClearSelections = true;//加载时是否清空选择项
	
	private boolean nowrap = true;//是否换行
...
//	update-end--Author:zhoujf  Date:20150608 for：修改增加easyui css主题目录切换的属性
	...
	public boolean isOnloadClearSelections() {
		return onloadClearSelections;
	}
	public void setOnloadClearSelections(boolean onloadClearSelections) {
		this.onloadClearSelections = onloadClearSelections;
	}
	/**
	 * 设置询问操作URL
	 */
	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
			...
			String style,String downloadName,boolean isAuto,String extendParams,String editor, String queryStyle) {
		...
		dataGridColumn.setQueryStyle(queryStyle);
...
//                update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
	...
	public StringBuffer end() {
		...
		if(onloadClearSelections){
			sb.append("$(\"#"+name+"\")."+grid+"(\"clearSelections\");");
		}
		...
		if (StringUtil.isNotEmpty(onCheckAll)) {
			sb.append("onCheckAll:function(rowDatas){" + onCheckAll + "(rowDatas);},");
		}
		if (StringUtil.isNotEmpty(onUncheckAll)) {
			sb.append("onUncheckAll:function(rowDatas){" + onUncheckAll + "(rowDatas);},");
		}
		...
		if(hasQueryColum(columnList)){
			sb.append("<div name=\"searchColums\" style=\"padding-top:10px;\">");
			...
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						...
						sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: 80px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; " + col.getQueryStyle() + "\" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			sb.append("<span style=\"float:right\">");
			sb.append("<input id=\""+name+"searchbox\" class=\"easyui-searchbox\" style=\"width:240px;height:28px;\"  data-options=\"searcher:"+name+ StringUtil.replaceAll("searchbox,prompt:\'{0}\',menu:\'#", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.input.keyword")) +name+"mm\'\"></input>");
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		/**
		 * TODO ZQD 注释原因，如果传入的分页数据行中【id】列没有数据时【opt】标签将会失效无法正常显示。
		 */
...
//            update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
			...
			if(column.getFormatterjs()!=null){
				...
				{
					sb.append(",formatter:function(value,rec,index){if (value == undefined) return '';");
			...
			if (columnStyleList.size() > 0 && !column.getField().equals("opt")) {
				...
				for (ColumnValue columnValue : columnStyleList) {
					if (columnValue.getName().equals(column.getField())) {
						String[] value = columnValue.getValue().split(",");
						String[] text = columnValue.getText().split(",");
						sb.append(",styler:function(value,rec,index){if (value == undefined) return '';");
						if((value.length == 0||StringUtils.isEmpty(value[0]))&&text.length==1){
							...
							for (int j = 0; j < value.length; j++) {
								testString += "if(value.indexOf('" + value[j] + "') != -1){return \'" + text[j] + "\'}";
							}
						}
						sb.append(testString);
						sb.append("}");
	...
	private String getEntityName() {
		if(StringUtils.isEmpty(entityName)){
			/**
			 * TODO ZQD 增加pojo自定义对象名
			 * 【注意:该对象一定要是泛型List<A> 中的A实体类名[因为转换时用到了实体类映射]，首字母小写 】
			 * 【应用场景影响】一个controller中含有分页查询时 
			 *   1）影响 datagrid 分页查询标签 生成，间接影响显示列筛选项fieId！
			 *   2） fieId查询列筛选，该数据项会存放在DataGrid.fieId中，参加分页数据转换（只取fieId指定成员变量转成 json数据返回）
			 */
			if(actionUrl.indexOf("pojo=")!=-1){
				String pojo = actionUrl.substring(actionUrl.indexOf("pojo=")+5);
				if(pojo.indexOf("&")!=-1){
