...
//	update-start--Author:longjb  Date:20150323 for：修改增加css主题类属性
	...
	protected boolean hasFileUpload = false;//是否有文件上传
	...
	protected boolean ajaxPost = true;
...
//			}
			...
			if(this.getTiptype()!=null && !"".equals(this.getTiptype())){
				...
				if(this.getTiptype().equals("lte")){//add by zbk
					sb.append("tiptype:function(msg,o,cssctl){");
					sb.append("layer.tips(msg,o.obj,{tips: [2, '#3595CC'],time: 3000});");
					sb.append("},");
				}else{
...
//			sb.append("},");
			...
			sb.append("ajaxPost:" + ajaxPost + ",");
			if (beforeSubmit != null && hasFileUpload == false) {
...
//				update-end--Author:gaofeng  Date:20140711 for：修改在使用jptransform时的逗号","拼接错误
			...
			if(hasFileUpload){
				...
				sb.append("curform.ajaxSubmit({");
				sb.append("type: 'POST',");
				sb.append("url: '" + action + "',");
				sb.append("success: function(data){");
				
				sb.append("if(frameElement.api != undefined){var win = frameElement.api.opener;var p = window.parent;");
...
//					sb.append("if(data.success==true){frameElement.api.close();win.tip(data.msg);}else{if(data.responseText==''||data.responseText==undefined){$.messager.alert('错误', data.msg);$.Hidemsg();}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息')); $.messager.alert('错误',emsg);$.Hidemsg();}catch(ex){$.messager.alert('错误',data.responseText+\"\");$.Hidemsg();}} return false;}");
				sb.append("if(data.success==true){frameElement.api.close();p.layer.msg(data.msg);}else{if(data.responseText==''||data.responseText==undefined){p.layer.msg(data.msg);}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息')); p.layer.msg(emsg);}catch(ex){p.layer.msg(data.responseText);}} return false;}");
				if (refresh) {
				...
				sb.append("}else{var win = window.parent;");
				sb.append("if(data.success==true){var index = parent.layer.getFrameIndex(window.name);parent.layer.close(index);win.layer.msg(data.msg);}else{");
				sb.append("if(data.responseText==''||data.responseText==undefined){layer.msg(data.msg);}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息'));layer.msg(emsg);}catch(ex){layer.msg(data.responseText);}}return false;}}");
