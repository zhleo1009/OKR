...
public class EhcacheAspect {
	...
	static {
		URL url = EhcacheAspect.class.getClassLoader().getResource("ehcache.xml");
		
		if (eternalCache == null) {
			eternalCache = CacheManager.create(url).getCache("eternalCache");
		}
		if (dictCache == null) {
			dictCache = CacheManager.create(url).getCache("dictCache");
		}
	}
