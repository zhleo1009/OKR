...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
	...
	public List<ComboTree> ComboTree(List all, ComboTreeModel comboTreeModel, List in, boolean recursive, boolean isOpen) {
		List<ComboTree> trees = new ArrayList<ComboTree>();
		for (Object obj : all) {
			trees.add(comboTree(obj, comboTreeModel, in, recursive, isOpen ? "open" : "closed"));
		}
		return trees;
	}
	...
	private ComboTree comboTree(Object obj, ComboTreeModel comboTreeModel, List in, boolean recursive) {
		return comboTree(obj, comboTreeModel, in, recursive, "closed");
	}
	 ...
	 * @param in
	 * @param recursive
	 * @param state	closed or open
	 * @return
	 */
	private ComboTree comboTree(Object obj, ComboTreeModel comboTreeModel, List in, boolean recursive, String state) {
		ComboTree tree = new ComboTree();
		...
		if (curChildList != null && curChildList.size() > 0) {
            ...
            if (recursive) { // 递归查询子节点
                ...
                for (Object childObj : nextChildList) {
                    ComboTree t = comboTree(childObj, comboTreeModel, in, recursive, state);
                    children.add(t);
                }
                tree.setChildren(children);
            }
