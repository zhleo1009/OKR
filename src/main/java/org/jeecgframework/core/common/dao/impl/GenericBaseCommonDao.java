...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	 ...
	 * @return
	 */
	public <T> T findUniqueByPropertyMap(Class<T> entityClass, 
			Map<String,Object> map) {
		return (T) createCriteriaMap(entityClass,map).uniqueResult();
	}
	
	
	
	private <T> Criteria createCriteriaMap(Class<T> entityClass,Map<String,Object> map) {
	...
	public List findByExample(final String entityName, final Object exampleEntity) {
		...
		Criteria executableCriteria = (entityName != null ? getSession().createCriteria(entityName) : getSession().createCriteria(
	...
	public Long getCountForJdbcParam(String sql, Object[] objs) {
		// -- update-begin author： xugj date:20160103 for: #851 controller
		// 单元测试升级spring 版本 -->
		return this.jdbcTemplate.queryForObject(sql, objs, Long.class);
		// -- update-end author： xugj date:20160103 for: #851 controller
		// 单元测试升级spring 版本 -->
	...
	public Object executeSqlReturnKey(final String sql, Map<String, Object> param) {
		...
		// update-begin--Author: jg_huangxg Date: 20150625
		// for：[bugfree号]oc时,录入数据在Oracle下 数据写入错误--------------------
		...
		// update-end--Author: jg_huangxg Date: 20150625
		// for：[bugfree号]oc时,录入数据在Oracle下 数据写入错误----------------------
		return keyValue;
	}
	...
	public <T> List<T>  findByHql(String hql, Map<String, Object> map, int pageSize, int pageNo) {
