...
import org.jeecgframework.core.enums.SysLTEIconEnum;
...
//		data = dataString.substring(0, dataString.toString().length()-1);
	...
	* @throws
	 ...
	 * @return
	 */
	public static String getLteMultistageTree(Map<Integer, List<TSFunction>> map) {
		...
		List<TSFunction> list = map.get(0);
            for (TSFunction function : list) {
                ...
                menuString.append("<a href=\"#\"><i class=\""+SysLTEIconEnum.toEnum(function.getTSIcon().getIconClas()).getThemes()+"\"></i>");
                menuString.append("<span>");
                menuString.append(getMutiLang(function.getFunctionName()));
                menuString.append("</span>");
                ...
                if(!function.hasSubFunction(map)){
                	...
                	//递归除了一级菜单之外的子节点
                	menuString.append("<i class=\"fa fa-angle-left pull-right\"></i></a>");//展开按钮
                	menuString.append(getLTESubMenu(function,1,map, new StringBuffer()));
                	menuString.append("</li>");
                }
            }
	 ...
	 * @return
	 */
	private static String getLTESubMenu(TSFunction parent,int level,Map<Integer, List<TSFunction>> map, StringBuffer menuString){
		...
		if(!CollectionUtils.isEmpty(list)){
			for (TSFunction function : list) {
				if (function.getTSFunction().getId().equals(parent.getId())){
					if(!function.hasSubFunction(map)){//无子菜单
						menuString.append(getLeafOfLTETree(function));
						
					}else{
						
						menuString.append("<li>");
						menuString
								.append("<a href=\"#\"><i class=\"fa fa-circle-o\"></i>");
						menuString.append("<span>  ");
						menuString.append(getMutiLang(function.getFunctionName()));
						menuString.append("</span>");
						menuString.append("<i class=\"fa fa-angle-left pull-right\"></i></a>");
						int subLevel = level + 1;
						getLTESubMenu(function, subLevel, map, menuString);
						menuString.append("</li>");
...
//		menuString.append(icon);
	 ...
	 * @return
	 */
	private static String getLeafOfLTETree(TSFunction function){
		StringBuffer menuString = new StringBuffer();
		String name = getMutiLang(function.getFunctionName()) ;
		menuString.append("<li> <a href=\"javascript:addTabs({id:\'").append(function.getId());
		menuString.append("\',title:\'").append(name).append("\',close: true,url:\'");
		...
		menuString.append(function.getFunctionUrl());
		menuString.append("\"  >");
		menuString.append("<i class=\"fa fa-circle-o\"></i>");
		menuString.append(name);
		menuString.append("</a></li>");
		return menuString.toString();
	}
