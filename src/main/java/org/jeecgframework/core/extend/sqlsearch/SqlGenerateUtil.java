...
public class SqlGenerateUtil {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static String generateDBUniqueFields(Object searchObj,String objSortName,String fields,List dealfields){
		StringBuffer dbFields = new StringBuffer();
		PropertyDescriptor[] propertyDescriptors = PropertyUtils.getPropertyDescriptors(searchObj.getClass());
		String[] fileNames = null;
		if(!StringUtil.isEmpty(fields)){
			fileNames=fields.split(",");
		}
		Set<String> fieldSet=new HashSet<String>();
		if(fileNames!=null){
			for(int i=0;i<fileNames.length;i++){
				if(!fieldSet.contains(fileNames[i])){
					fieldSet.add(fileNames[i]);
					for(PropertyDescriptor propertyDescriptor:propertyDescriptors){
						String propertyName = propertyDescriptor.getName();
						if(fileNames[i].equals(propertyName)){
							dbFields.append((StringUtil.isEmpty(objSortName)?"":(objSortName+"."))+getDbNameByFieldName(propertyDescriptor)+" as "+fileNames[i]+((i==fileNames.length-1)?"":","));
							if(dealfields!=null){
								dealfields.add(fileNames[i]);
							}
							break;
			...
			dbFields.append(" select ");
			for(PropertyDescriptor propertyDescriptor:propertyDescriptors){
				String propertyName = propertyDescriptor.getName();
				if(!"class".equalsIgnoreCase(propertyName)){
					dbFields.append((StringUtil.isEmpty(objSortName)?"":(objSortName+"."))+getDbNameByFieldName(propertyDescriptor)+" as "+propertyName+",");
				}
			}
			dbFields=new StringBuffer(dbFields.toString().substring(0, dbFields.length()-1));
			dbFields.append(" from "+SqlGenerateUtil.generateTable(searchObj)+" "+(StringUtil.isEmpty(objSortName)?"":objSortName)+" ");
			dbFields.append(SqlGenerateUtil.generateLikeWhere(searchObj,null));
		}
		
		if(dbFields.toString().endsWith(",")){
			return dbFields.toString().substring(0, dbFields.length()-1);
		}else{
			return dbFields.toString();
		}
	}
	 ...
	 * @return
	 */
	public static StringBuffer generateLikeWhere(Object searchObj,Map<String,String[]> parameterMap){
			...
			for(PropertyDescriptor propertyDescriptor:propertyDescriptors){
				...
				if (parameterMap != null && (parameterMap.containsKey(propertyName + BEGIN)||parameterMap.containsKey(propertyName + END))){
					...
					if(StringUtil.isNotEmpty(propertyValue)){
						...
						if(StringUtil.isNotEmpty(propertyValueReturn)&&propertyValueReturn.indexOf("%")==-1){
							whereSql.append("and "+dbColumnName+"="+propertyValueReturn+" ");
						}else if(StringUtil.isNotEmpty(propertyValueReturn)&&propertyValueReturn.indexOf("%")!=-1){
							whereSql.append("and "+dbColumnName+" like "+propertyValueReturn+" ");
						}
					}
				}
			}
