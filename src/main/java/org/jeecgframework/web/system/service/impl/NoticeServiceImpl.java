...
public class NoticeServiceImpl extends CommonServiceImpl implements NoticeService {
	...
	public <T> void delete(T entity) {
		// update-begin--Author:xuguojie Date:20160406 for：#1020
		// 【平台bug】通知公告删除时，没有考虑t_s_notice_read_user表的记
	...
	@Override
	public List<Map<String, Object>> getNoticeList(String userId, int page, int size) {

		String sql = "";
		sql += " SELECT * FROM  t_s_notice t WHERE";
		sql += " (t.notice_level = '1' ";
		sql += " OR (t.notice_level = '2' AND EXISTS (SELECT 1 FROM t_s_notice_authority_role r,t_s_role_user ru WHERE r.role_id = ru.roleid AND t.id = r.notice_id AND ru.userid = '"
				+ userId + "'))";
		sql += " OR (t.notice_level = '3' AND EXISTS (SELECT 1 FROM t_s_notice_authority_user u WHERE t.id = u.notice_id AND u.user_id = '" + userId
				+ "'))";
		sql += " ) AND NOT EXISTS (SELECT 1 FROM t_s_notice_read_user rd WHERE t.id = rd.notice_id AND rd.user_id = '" + userId + "') AND t.is_delete = 0 ";
		sql += " ORDER BY t.create_time DESC ";
		List<Map<String, Object>> noticeList = findForJdbc(sql, page, size);
		return noticeList;
	}

	@Override
	public List<Map<String, Object>> getAllNoticeList(String userId, int page, int size) {
