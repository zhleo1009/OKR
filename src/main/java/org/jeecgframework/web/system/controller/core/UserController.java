...
public class UserController extends BaseController {
	...
	//update--start--by:jg_renjie--at:20160318 for:#942 【组件封装】组织机构弹出模式，目前是列表，得改造成树方式
		@RequestMapping(params = "getUsernameById")
		@ResponseBody
...
//			update-start--Author:longjb  Date:20150408 for：ACE风格未指定情况下主题为metro
    ...
    @ResponseBody
   public   List<TSUser>   getCombobox()  
   {  
       List<TSUser> list = systemService.getList(TSUser.class);
       return  list;  
   }  
     ...
     * @return
     */
    @RequestMapping(params = "getRealNameById")
    @ResponseBody
    public AjaxJson getRealNameById(TSUser user, HttpServletRequest request) {
        AjaxJson j = new AjaxJson();
        
        String id = request.getParameter("id");
        TSUser t = systemService.get(TSUser.class, id);
        try {
            if(null!=t)
            j.setMsg(t.getRealName());
        } catch (Exception e) {
            e.printStackTrace();
