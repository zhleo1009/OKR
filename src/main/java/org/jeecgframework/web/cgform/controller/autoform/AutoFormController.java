...
//	}
	...
	@ResponseBody
	public com.alibaba.fastjson.JSONObject checkTbCode(HttpServletRequest request, String formName){
		...
		try {
			...
			String hql = "from AutoFormEntity t where t.formName = ?";
			list = this.systemService.findHql(hql, formName);

			if(list.size()>0){
				jsonObject.put("success", false);
				jsonObject.put("msg", "编码重复，请重新输入！");
				return jsonObject;
			}
