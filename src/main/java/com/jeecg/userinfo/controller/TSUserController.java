...
public class TSUserController extends BaseController {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(TSUserController.class);
	...
	public ModelAndView list(HttpServletRequest request) {
		return new ModelAndView("com/jeecg/userinfo/tSUserList");
	}
	...
	public ModelAndView goAdd(TSUserEntity tSUser, HttpServletRequest req) {
		...
		return new ModelAndView("com/jeecg/userinfo/tSUser-add");
	}
	/**
	 * t_s_user编辑页面跳转
	 * 
	...
	public ModelAndView goUpdate(TSUserEntity tSUser, HttpServletRequest req) {
		...
		return new ModelAndView("com/jeecg/userinfo/tSUser-update");
	}
	...
	public String exportXls(TSUserEntity tSUser,HttpServletRequest request,HttpServletResponse response
		...
		modelMap.put(NormalExcelConstants.FILE_NAME,"t_s_user");
		modelMap.put(NormalExcelConstants.CLASS,TSUserEntity.class);
		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("t_s_user列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
			"导出信息"));
	...
	@ResponseBody
	public ResponseEntity<?> create(@RequestBody TSUserEntity tSUser, UriComponentsBuilder uriBuilder) {
		...
		String id = tSUser.getId();
		URI uri = uriBuilder.path("/rest/tSUserController/" + id).build().toUri();
	...
	public ResponseEntity<?> update(@RequestBody TSUserEntity tSUser) {
		//调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
		Set<ConstraintViolation<TSUserEntity>> failures = validator.validate(tSUser);
		if (!failures.isEmpty()) {
			return new ResponseEntity(BeanValidators.extractPropertyAndMessage(failures), HttpStatus.BAD_REQUEST);
